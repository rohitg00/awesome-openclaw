...
| **[Molted](https://www.molted.net)** | Per instance/day (not monthly; prorated) | ~2 min | Managed K8s | Built for fleets: auto-healing (crash detection under 60s, recovery under 90s, post-mortem per failure), versioned files with point-in-time restore, per-instance version pinning and rollback, per-agent email/phone/browser automation, 1,000+ integrations, API to manage every instance | - |
